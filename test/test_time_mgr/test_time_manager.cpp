...
{
  When( Method( Solar_Events_Mock, GetNextEvents ) )
          .AlwaysDo(
                  [&]( const time_t& aTime, time_t& aSunrise, time_t& aSunset )
                  {
                    if( aTime == Now )
                    {
                      aSunrise = Sunrise;
...
class TEST_TIME_MGR
{
  ...
  public:
  ~TEST_TIME_MGR() { delete m_Time_Mgr; }

  void Init() { m_Time_Mgr->Init( DayDuration ); }
  ...
  void AtSunsetShouldGetNewSolarEvents()
  {
    ...
    Verify( Method( Solar_Events_Mock, GetNextEvents )
            + OverloadedMethod( ArduinoFake( Serial ), print, size_t( char ) ).Using( '9' ) )
            .Twice();
  }
  ...
  void AtSunsetHardwareFailureShouldGoToSleep()
  {
    ...
    Verify( OverloadedMethod( ArduinoFake( Serial ), print, size_t( char ) ).Using( '9' ) )
            .Twice(); // sleep command is called at init() and sunset
  }
